import com.olympus.oir.parser.OirParser;
import com.olympus.oir.model.ParsedOirFile;
import com.olympus.oir.model.OirBlock;
import java.io.File;
import java.io.RandomAccessFile;
import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.util.Map;
import java.util.List;

public class TestPixelValues {
    public static void main(String[] args) throws Exception {
        File f = new File("D:\\phase-01\\assignment1-phase1.oir");
        OirParser parser = new OirParser(f);
        ParsedOirFile parsed = parser.parse();
        
        Map<String, Integer> map = parsed.getFragmentIndexMap();
        List<OirBlock> blocks = parsed.getBlocks();
        
        String firstKey = map.keySet().iterator().next();
        int blockNo = map.get(firstKey);
        
        OirBlock target = blocks.get(blockNo);
        OirBlock next = blocks.get(blockNo + 1);
        
        if (next.isImageBitmap()) {
            try (RandomAccessFile raf = new RandomAccessFile(next.getSourceFile(), "r")) {
                raf.seek(next.getDataOffset());
                byte[] fragBytes = new byte[next.getDataSize()];
                raf.readFully(fragBytes);

                ByteBuffer buf = ByteBuffer.wrap(fragBytes).order(ByteOrder.LITTLE_ENDIAN);
                int max = 0;
                int min = 65535;
                int nonZeroCount = 0;
                while (buf.remaining() >= 2) {
                    int val = buf.getShort() & 0xFFFF;
                    if (val > max) max = val;
                    if (val < min) min = val;
                    if (val > 0) nonZeroCount++;
                }
                System.out.println("Pixels > 0: " + nonZeroCount);
                System.out.println("Min: " + min);
                System.out.println("Max: " + max);
            }
        }
    }
}
